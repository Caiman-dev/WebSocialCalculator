...
let tarif = [
    ...
    [200, 300, 300, 300, 300, 300, 400, 400, 600, 600, 900, 1300, 1600, 1900, 2100, 2300, 2500, 2700, 3000, 3500, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 250, 250, 250, 250, 250, 450, 550, 650, 750, 950, 1400, 1800, 2100, 2300, 2600, 3000, 3400, 3800, 4000, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [100, 200, 100, 100, 100, 100, 100, 100, 200, 200, 200, 300, 300, 300, 300, 350, 400, 400, 600, 800, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    ...
    [300, 500, 500, 500, 500, 500, 700, 800, 1100, 1200, 1700, 2500, 3100, 3700, 4100, 4500, 5000, 5400, 6000, 6500, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [200, 300, 300, 300, 300, 300, 400, 400, 600, 600, 900, 1300, 1600, 1900, 2100, 2300, 2500, 2700, 3000, 3500, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    ...
    ['нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 250, 250, 250, 250, 250, 450, 550, 650, 750, 950, 1400, 1800, 2100, 2300, 2600, 3000, 3400, 3800, 4000, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 250, 200, 200, 200, 200, 300, 300, 400, 400, 500, 600, 700, 700, 900, 1000, 1100, 1300, 1400, 1600, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 300, 250, 250, 250, 250, 350, 350, 450, 450, 650, 700, 800, 800, 1100, 1200, 1300, 1400, 1600, 1850, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [200, 350, 300, 300, 300, 300, 400, 400, 550, 650, 750, 800, 900, 1000, 1300, 1400, 1500, 1600, 1800, 2100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [200, 350, 300, 300, 300, 300, 400, 400, 550, 650, 750, 800, 900, 1000, 1300, 1400, 1500, 1600, 1800, 2100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [400, 700, 650, 650, 650, 650, 800, 800, 1100, 1300, 1500, 1600, 1800, 2000, 2600, 2800, 3000, 3200, 3600, 4200, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [450, 650, 550, 650, 700, 800, 1050, 1150, 1450, 1550, 2050, 2700, 3300, 3900, 4600, 5300, 5900, 6700, 7600, 9000, 42, 40, 39, 39, 38, 220000],
    [300, 500, 400, 500, 550, 650, 800, 1000, 1200, 1400, 1800, 2400, 2900, 3500, 4200, 4800, 5400, 6000, 6800, 7500, 38, 37, 36, 36, 35, 188000],
    [250, 350, 250, 350, 400, 500, 650, 750, 850, 950, 1150, 1400, 1700, 2000, 2500, 3000, 3400, 4000, 4600, 5500, 26, 24, 23, 23, 22, 105000],
    [150, 300, 250, 250, 250, 250, 350, 350, 450, 450, 650, 700, 800, 800, 1100, 1200, 1300, 1400, 1600, 1850, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 250, 200, 200, 200, 200, 300, 300, 400, 400, 500, 600, 700, 700, 900, 1000, 1100, 1300, 1400, 1600, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [200, 350, 300, 300, 300, 300, 400, 400, 550, 650, 750, 800, 900, 1000, 1300, 1400, 1500, 1600, 1800, 2100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [200, 400, 350, 350, 350, 350, 500, 500, 600, 600, 900, 1000, 1200, 1200, 1600, 1800, 2000, 2200, 2600, 3100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [400, 750, 700, 700, 700, 700, 900, 900, 1150, 1250, 1650, 1800, 2100, 2200, 2900, 3200, 3500, 3800, 4400, 5200, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [450, 700, 550, 650, 700, 800, 1050, 1150, 1450, 1550, 2050, 2800, 3400, 4000, 4800, 5500, 6100, 6800, 7800, 9250, 42, 40, 39, 39, 38, 225000],
    [350, 600, 450, 550, 600, 700, 950, 1150, 1350, 1550, 1950, 2700, 3300, 3900, 4700, 5400, 6100, 6800, 7800, 8750, 40, 39, 39, 39, 39, 203000],
    [250, 400, 250, 350, 400, 500, 650, 750, 850, 950, 1150, 1500, 1800, 2100, 2700, 3200, 3600, 4100, 4800, 5750, 26, 24, 23, 23, 22, 110000],
    [200, 350, 300, 300, 300, 300, 400, 400, 550, 650, 750, 800, 900, 1000, 1300, 1400, 1500, 1600, 1800, 2100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [200, 350, 300, 300, 300, 300, 400, 400, 550, 650, 750, 800, 900, 1000, 1300, 1400, 1500, 1600, 1800, 2100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 250, 200, 200, 200, 200, 300, 300, 400, 400, 500, 600, 700, 700, 900, 1000, 1100, 1300, 1400, 1600, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [250, 450, 400, 400, 400, 400, 550, 550, 700, 800, 1000, 1100, 1300, 1400, 1800, 2000, 2200, 2400, 2800, 3350, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [450, 800, 750, 750, 750, 750, 950, 950, 1250, 1450, 1750, 1900, 2200, 2400, 3100, 3400, 3700, 4000, 4600, 5450, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [500, 750, 550, 650, 700, 800, 1050, 1150, 1450, 1550, 2050, 2900, 3500, 4200, 5000, 5700, 6300, 7000, 8000, 9500, 44, 42, 41, 41, 40, 230000],
    [400, 650, 450, 550, 600, 700, 950, 1150, 1350, 1550, 1950, 2800, 3400, 4100, 4900, 5600, 6300, 7000, 8000, 9000, 42, 41, 41, 41, 41, 208000],
    [300, 450, 250, 350, 400, 500, 650, 750, 850, 950, 1150, 1600, 1900, 2300, 2900, 3400, 3800, 4300, 5000, 6000, 28, 26, 25, 25, 24, 115000],
    [200, 350, 300, 300, 300, 300, 400, 400, 550, 650, 750, 800, 900, 1000, 1300, 1400, 1500, 1600, 1800, 2100, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    ...
    [250, 450, 400, 400, 400, 400, 550, 550, 700, 800, 1000, 1100, 1300, 1400, 1800, 2000, 2200, 2400, 2800, 3350, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [150, 250, 200, 200, 200, 200, 300, 300, 400, 400, 500, 600, 700, 700, 900, 1000, 1100, 1300, 1400, 1600, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [300, 550, 500, 500, 500, 500, 600, 600, 850, 950, 1150, 1200, 1300, 1400, 1900, 2000, 2100, 2200, 2400, 2700, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
    [500, 750, 550, 650, 700, 800, 1050, 1150, 1450, 1550, 2050, 2900, 3500, 4200, 5000, 5700, 6300, 7000, 8000, 9500, 42, 40, 40, 40, 38, 220000],
    [300, 500, 400, 500, 550, 650, 800, 1000, 1200, 1400, 1800, 2400, 2900, 3500, 4200, 4800, 5400, 6000, 6800, 7500, 38, 37, 36, 36, 35, 188000],
    [300, 450, 250, 350, 400, 500, 650, 750, 850, 950, 1150, 1600, 1900, 2300, 2900, 3400, 3800, 4300, 5000, 6000, 26, 24, 24, 24, 22, 105000],
    [400, 700, 650, 650, 650, 650, 800, 800, 1100, 1300, 1500, 1600, 1800, 2000, 2600, 2800, 3000, 3200, 3600, 4200, 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета', 'нет расчета'],
...
$("#length").on('change', function () {
    if (length.value != "" && length.value != "" &&
        ...
        var volValue = length.value * width.value * height.value;
...
function GetCol() {
    if (cargoType == 'Письмо') {
        ...
        maxValue = 0;
        if (Number(weightValue) > Number(volumeValue) || Number(weightValue) == Number(volumeValue)) {
            maxValue = Number(weightValue);
        }
        else if (Number(weightValue) < Number(volumeValue)) {
            maxValue = Number(volumeValue);
        }
        // maxValue = weightValue > volumeValue ? weightValue : volumeValue;
        // console.log('weightValue: ' + weightValue + '/n');
        // console.log('volumeValue: ' + volumeValue + '/n');
        // console.log('maxValue: ' + maxValue + '/n');
